...
We are certain that there are differences among the varieties, and we should look at all of the pairwise contrasts among the variety levels. As usual we could use the package `lsmeans`, which automates much of this (and uses `lmerTest` produced p-values for the tests).

```{r, warning=FALSE, message=FALSE}
# currently lmerTest and lsmeans packages fight and we have to specify which package to use
LetterResults <- lsmeans::lsmeans( model.1, pairwise ~ variety) %>% cld(Letters=letters)
LetterResults
```
