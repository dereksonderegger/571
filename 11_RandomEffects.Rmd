...
library(lsmeans)    # Pairwise contrasts
...
```{r}
# car::Anova(model.1, type=3, test.statistic='F')  # Using Wald F tests; won't give a LRT
car::Anova(model.1, type=3)  # Type III anova table (just as we've had before)
```

By default, `car::Anova() is the Wald Chi-Square test instead of the LRT, and should be reasonable. Using `test.statistic='F'` will give another test, but `car::Anova()` refuses to do a LRT because the `car` package authors believe that the Wald tests are more appropriate. There is quite a bit of debate among statisticians about which test should be recommended and this is an active area of research. Recent work suggests that permutate based tests are most widely applicable, but reasonably general software has not yet emerged (to my knowledge). Fortunately in this case, we don't see any qualitative difference because the variety term is still statisitcally significant.
...
We are certain that there are differences among the varieties, and we should look at all of the pairwise contrasts among the variety levels. As usual we could use the package `lsmeans`, which automates much of this.

```{r, warning=FALSE, message=FALSE}
LetterResults <- lsmeans( model.1, pairwise ~ variety) %>% cld(Letters=letters)
LetterResults
```
...
```{r}
lsmeans(m, specs=pairwise~material) %>% cld(Letters=letters) 
```
