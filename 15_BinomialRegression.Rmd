...
We use the notation $y_{i}=\boldsymbol{X}_{i,\cdot}\boldsymbol{\beta}$ to denote a single set of covariate values and notice that this is unconstrained and can be in $\left(-\infty, +\infty\right)$ while the parameter of interest $p_{i}=g^{-1}\left(y_{i}\right)$ is constrained to $\left[0,1\right]$. When convenient, we will drop the $i$ subscript while keeping the domain restrictions.

There are several options for the link function $g^{-1}\left(\cdot\right)$ that are commonly used. 

1. Logit (log odds) transformation. The link function is $$g\left(p\right)=\textrm{logit}(p)=\log\left[\underset{\textrm{odds}}{\underbrace{\frac{p}{1-p}}}\right]=y$$
 with inverse $$g^{-1}\left(y\right)=\textrm{ilogit}(y)=\frac{1}{1+e^{-y}}=p$$ and we think of $g\left(p\right)$ as the log odds function. 
...
data.frame( x= seq(-5, 5, by=.01)) %>%
  mutate(y = faraway::ilogit(x)) %>%
  ggplot(aes(x=x, y=y)) +
  geom_line(color='red') +
  labs(x=latex2exp::TeX('y = $X\\beta$'), y='p', title='Inverse Logit() function')
```
...
$$w_i \stackrel{iid}{\sim} Bernoulli(p_i)$$ 

Then

$$\mathcal{L}(\boldsymbol{p}|\boldsymbol{w}) = \prod_{i=1}^n p_i^{w_i}(1-p_i)^{1-w_i}$$
and therefore  substituting $p_i = \textrm{ilogit}(\boldsymbol{X}_i\boldsymbol{\beta})$ in we have


$$\begin{aligned}
 \mathcal{L}\left(\boldsymbol{\beta}|\boldsymbol{w} \right)	&=
	\prod_{i=1}^n \left(\textrm{ilogit}(\boldsymbol{X}_i \boldsymbol{\beta}) \right)^{w_i} 
	              \left( 1-\textrm{ilogit}(\boldsymbol{X}_i \boldsymbol{\beta})\right)^{1-w_i}  
	\end{aligned}$$
and

$$\log \mathcal{L}\left(\boldsymbol{\beta}|\boldsymbol{w} \right)	= 
\sum_{i=1}^n \log \left\{ 
  \left(   \textrm{ilogit}(\boldsymbol{X}_i \boldsymbol{\beta})\right)^{w_i} 
  \left( 1-\textrm{ilogit}(\boldsymbol{X}_i \boldsymbol{\beta})\right)^{1-w_i}
\right\}$$

Often times it is more numerically stable to maximize the log-likelihood rather than the pure likelihood function because using logs helps prevent machine under-flow issues when the values of the likelihood is _really_ small, but we will ignore that here and just assume that the function the performs the maximization is well designed to consider such issues.

We have more than one response at a particular level of $\boldsymbol{X}$. Let $m_{i}$ be the number of observations observed at the particular value of $\boldsymbol{X}$, and $w_{i}$ be the number of successes at that value of $\boldsymbol{X}$. In that case, $w_{i}$ is not a Bernoulli random variable, but rather a binomial random variable. Note that the Bernoulli distribution is the special case of the binomial distribution with $m_{i}=1$.
...
because it provided a mechanism for comparing the fit of two different models. If a model had a very small SSE, then it fit the observed data well. We used this as a basis for forming our F-test to compare nested models (some re-scaling by the appropriate degrees of freedom was necessary, though).
...
## Exercises {#Exercises_BinomialRegression}
