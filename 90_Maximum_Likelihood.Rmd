...
### Exponential Example
We next consider data sampled from the exponential distribution. For this example, suppose that we had observed $\textbf{x}=\{7.2, 6.1, 9.0, 12, 8.8\}$ 


$$\mathcal{L}(\mu, \sigma | \textbf{x}) = \prod_{i=1}^n f(x_i|\mu,\sigma) = \prod_{i=1}^n \beta e^{\beta x_i}$$
which is maximized at $\hat{\mu} = \bar{x} = 8.62$. We can numerically see this following the same process as previously seen.

```{r, echo=FALSE, fig.height=5}
x <- c(7.2, 6.1, 9.0, 12, 8.8)
mean(x)

data <- 
  expand.grid( beta=seq(0,30, by=0.01), x=x) %>%
...
P1 <- 
  ...
  geom_line() +
  annotate('path', x=c(8.62,8.62), y=c(1.415e-7, 0), color='blue') +
  annotate('text', x=8.62, y=1.52e-7, label=latex2exp::TeX('$\\hat{\\beta}$'), color='blue') +
...
P2 <- 
  ...
  geom_line() +
  annotate('path', x=c(8.62,8.62), y=c(-15.8, -30), color='blue') +
  annotate('text', x=8.62, y=-14.8, label=latex2exp::TeX('$\\hat{\\beta}$'), color='blue') +
  scale_y_continuous(limits=c(-30,-14.5)) +
  labs( x = latex2exp::TeX('$\\beta$'), 
        y = latex2exp::TeX('$logL(\\, \\lambda \\,|\\, \\textbf{x} \\,)$'),
...
x <- c(7.2, 6.1, 9.0, 12, 8.8)
neglogL <- function(param){
  dexp(x, rate=1/param) %>%   # We defined beta as 1/rate
    log() %>% sum() %>% prod(-1) %>%
    return()
}
...
while the spread of the distribution is inversely related to the magnitude of $\alpha$ and $\beta$. The density function $f(x|\alpha,\beta)$ can be accessed in R using the `dbeta(x, alpha, beta)` function.
    ...
    d) Show that the maximum likelihhood estimators of $\alpha$ and $\beta$ are approximately $9.5$ and $12.5$.
