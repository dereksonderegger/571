...
m1 <- glm( (Occupancy == 1)              ~ CCU, data=Mayflies, family=binomial )
```
...
# Lets get this value through lsmeans() instead.
lsmeans::lsmeans(m1, ~CCU, type='response', at=list(CCU=1))
...
Suppose that we were to give a predicted Presense/Absense class based on the $\hat{p}$ value. Lets predict presense if the probability is greater than 0.5 and absent if the the probability is less that 0.5. 
...
This scheme has misclassified 3 observations, two cases where mayflies were present but we predicted they would be absent, and one case where no mayflies were detected but we predicted we would see them.
...
```{r}
m0 <- glm( (Occupancy==1) ~ 1, data=Mayflies, family=binomial )
anova(m0, m1)
...
Alternatively, we might want to do this calculation via `lsmeans`.

```{r}
lsmeans::lsmeans(m1, ~conc, at=list(conc=c(0:4)), type='response') 
```
...
```{r}
lsmeans(m2, pairwise~period) %>% cld(Letters=letters)
```
