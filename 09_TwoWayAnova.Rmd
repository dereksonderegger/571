...
```{r}
m3 <- lm(Yield ~ Var + Pest, data=fruit)
lsmeans(m3, spec=pairwise~Var)
lsmeans(m3, spec=pairwise~Pest)
```
...
```{r}
m3 <- lm(Yield ~ Var + Pest, data=fruit)
lsmeans(m3, spec=pairwise~Var) %>% cld(Letters=letters)
lsmeans(m3, spec=pairwise~Pest) %>% cld(Letters=letters)
```
...
In the unbalanced case taking the average of the cell means produces a different answer than taking the average of the data. The `lsmeans` package chooses to take the average of the cell means.
