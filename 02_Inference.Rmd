...
<<fig.height=4>>=
fish <- read.table('~/Dropbox/NAU/Teaching/STA 571/Notes/c1.3_Inference/Fish.csv',                    header=TRUE, skip=111, sep=',') pairs(fish[,c('Weight','Length.1','Length.2','Length.3','Height','Width')])@
...
In this case the sum of the three length variables was approximately 31
  in both cases, but with three length variables, the second could be negative the third be positive with approximately the same magnitude and we get approximately the same model as with both the second and third length variables missing from the model.
